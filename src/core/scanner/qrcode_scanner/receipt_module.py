    ...
    try:
        ...
        def decode_by_flag(decoded_objects, flag):
            ...
            try:
                if flag == "0":  # big5
                    ...
                    content_obj_0 = decoded_objects[0].data.decode(
                        "utf-8", errors="replace"
                    )
                    content_obj_1 = (
                        decoded_objects[1].data.decode("utf-8", errors="replace")
                        if len(decoded_objects) > 1
                        else ""
                    )
        ...
        if ok:
            ...
            for i in range(len(data)):
                ...
                # 在原始彩色圖片上繪製矩形框
                cv2.rectangle(
                    img_original, (box[0], box[1]), (box[2], box[3]), (0, 0, 255), 5
                )

                # 調整文字位置，避免重疊
                # 使用 img_original 放置文字
                putText(
                    img_original, box[0] + 10, box[3] + 25, text, color=(0, 255, 0)
                )  # 07041322在圖片上寫出文本，增加偏移量
        ...
        if len(parts) > 3:
            ...
            if decoded_objects:
                ...
                encoding, qrcode_all = decode_by_flag(
                    decoded_objects, encoding_flag
                )  # 2025/7/7 17:11 test
        ...
        encoding, decoded_content = decode_by_flag(
            decoded_objects, encoding_flag
        )  # 2025/7/7 17:07
        ...
        with open(
            "invoice_data.csv", "a", newline="", encoding=encoding
        ) as csvfile:  # 2025/7/7 16:33
